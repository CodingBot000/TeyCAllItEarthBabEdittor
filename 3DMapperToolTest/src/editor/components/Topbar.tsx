...
interface TopbarProps {
  ...
  onModeChange: (mode: 'building' | 'asset' | 'scene') => void;
...
export function Topbar({ mode, onModeChange, definition, buildingId, onBuildingChange, onOpen, onImportGlb, onSave, onExport, onExportPackage, canUndo, canRedo, onUndo, onRedo }: TopbarProps) {
  return (
    <header className="topbar">
      ...
      <label className="topbar-mode-select">
        <span className="sr-only">Select editor mode</span>
        <select value={mode} onChange={(event) => onModeChange(event.target.value as TopbarProps['mode'])}>
