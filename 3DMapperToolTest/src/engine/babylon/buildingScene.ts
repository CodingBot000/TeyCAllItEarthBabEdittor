...
export function createBuildingScene(
  ...
  const getSceneTree = (): SceneTreeNode[] => {
    ...
    return [
      { id: 'glb-root', name: importedModelName ?? 'Imported GLB', type: 'group', depth: 0, imported: true },
      ...importedMeshes.map((mesh) => ({ id: mesh.metadata?.treeId as string, name: mesh.name || 'Unnamed Mesh', type: 'mesh' as const, depth: 1, imported: true })),
    ];
  };
  ...
  const selectSceneNode = (id: string) => {
    ...
    if (face && FACE_KEYS.includes(face)) {
      ...
      const mesh = importedMeshes.find((candidate) => candidate.metadata?.treeId === id);
      if (mesh) {
  ...
  const loadGlb = async (file: File): Promise<string> => {
    ...
    try {
      ...
      const result = await SceneLoader.ImportMeshAsync('', objectUrl, '', scene, undefined, '.glb', file.name);
      importedRoot = new TransformNode(`ImportedGLB-${file.name}`, scene);
      importedRoot.metadata = { treeId: 'glb-root', imported: true, export: true };
      importedModelName = file.name;
      importedMeshes = result.meshes.filter((mesh): mesh is AbstractMesh => mesh instanceof AbstractMesh);
      importedMeshes.forEach((mesh, index) => {
        mesh.parent = importedRoot;
        mesh.metadata = { ...(mesh.metadata ?? {}), treeId: `glb-mesh-${index}`, imported: true, export: true };
