...
## 20. 2026-08-26 — 레퍼런스 기준 하단 확장 보정

- 실제 레퍼런스와 게임 캡처 비교 후, 구현값의 지면 반폭을 6.5에서 15로 조정했다.
- 모선 흡입구 쪽 반폭 4.2는 유지하고 지면 쪽만 크게 확장해, 광선이 항상 위쪽은 좁고 아래쪽은 넓은 깔때기 형태로 보이게 했다.
- IGNITING 단계에서도 폭을 0.9 이하로 줄이지 않아 점화 첫 프레임부터 하단 확장 형태를 유지한다.
- 이 조정은 흡입광선의 외형·halo·내부 가상 오브젝트 분산 폭만 변경하며 실제 흡수 판정과 게임 오브젝트 위치에는 영향을 주지 않는다.

## 21. 2026-08-26 — 가상 실루엣 크기 및 흡수 시간 조정

- 사용자가 확인한 화면에서 흡입광선 내부 사람 실루엣이 너무 작아 보이는 문제를 반영해 렌더 크기 범위를 `0.45~1.1`에서 `0.9~2.1`로 확대했다. 파일 자체는 320×128, 약 7.6KB 투명 WebP를 유지한다.
- 개별 가상 실루엣이 바닥에서 모선까지 이동하는 0.8초는 유지한다. 실제 대상의 전체 흡수 속도는 `durationMultiplier=3`으로 조정해 기존의 3배 시간이 걸리게 했다.
- 흡수 미리보기의 예상 시간과 실제 `tickBeam` 차감 속도에 같은 배율을 적용해 HUD 예측과 실제 진행이 어긋나지 않게 했다.
- 확대·시간 조정 후 흡수 전용 브라우저 E2E, 전체 Vitest 103/103, typecheck, production build, 관련 ESLint를 통과했다.
