...
## 2026-08-27 — 함선 무적 디버그 버튼 단독 노출

- 개발용 전투 상단에는 `함선 무적 디버그` 버튼만 표시하고, 포인트 디펜스·유닛 무적·배경/충돌 디버그 버튼은 숨겼다.
- 함선 무적 디버그의 초기 상태는 `OFF`이며, 기존 무적 토글 동작과 내부 디버그 로직은 삭제하지 않고 유지했다.
...
## 2026-08-27 — 선택형 전투 효과음 활성화

- 전투 효과음은 EMP, 오버드라이브, 플라즈마, 인간 흡입 광선만 허용하도록 선택형 토글로 변경했다.
- 방공 레이저 효과음은 계속 비활성화하며, 오버드라이브는 제거된 실드 피격음을 되살리지 않고 현재 펄스 계열 효과음을 재사용한다.
- `playAbilitySound`에 오버드라이브 경로를 추가하고 흡입 루프는 기존 시작/종료 동기화를 유지했다.

## 2026-08-27 — 중량 능력 과충전 셀 제한 제거 및 에너지 비용 통일

- EMP, 플라즈마, 오버드라이브는 과충전 셀을 요구하거나 차감하지 않고, 에너지 400을 소비하도록 변경했다.
- 기존 쿨다운과 EMP/플라즈마의 28m 이내 자동 표적 조건은 유지해 에너지와 전투 상황이 사용 제한으로 작동한다.
- 능력 가용성·도메인 비용 테스트를 갱신해 셀 0개에서도 세 버튼이 활성화되고 사용 후 에너지가 400 감소하는 것을 검증한다.
...
## 2026-08-27 — EMP·플라즈마·오버드라이브 에너지 비용 통일

- 세 중량 능력은 과충전 셀을 요구하거나 차감하지 않고, 사용마다 에너지 400을 소비하도록 변경했다.
- 기존 EMP/플라즈마의 유효 표적·28m 사거리와 세 능력의 쿨다운은 유지했다. 따라서 “항상 사용”은 셀 제한 없이 에너지와 전투 조건이 충족될 때마다 사용 가능하다는 의미다.
- 셀 0개 활성화·400 에너지 차감 단위 테스트 10개, typecheck, production build, 표준 브라우저 캡처를 통과했다. 장시간 전투기 테스트는 전체 병렬 실행에서 일시적으로 타임아웃됐으나 단독 31/31 재실행은 통과했다.
