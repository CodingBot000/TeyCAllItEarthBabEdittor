...
const useNaturalFailure = process.env.SIDE_VIEW_NATURAL_FAILURE === '1';
...
try {
  ...
  const beforeDestruction = await page.evaluate(() => JSON.parse(window.render_game_to_text?.() ?? 'null'));
  await page.getByTestId('battle-invincibility-toggle').click();
  let destructionStart = null;
  if (useNaturalFailure) {
    await page.getByTestId('battle-point-defense-toggle').click();
    for (let second = 0; second < 180; second += 1) {
      await page.evaluate(() => window.advanceTime?.(1000));
      const state = await page.evaluate(() => JSON.parse(window.render_game_to_text?.() ?? 'null'));
      if (state?.result === 'FAILED') {
        destructionStart = state;
        break;
      }
    }
    if (!destructionStart) throw new Error('Natural enemy damage did not disable the mothership within 180 simulated seconds.');
  } else {
    await page.keyboard.press('2');
  }
  const elapsedAtDetection = destructionStart?.mothershipDestruction?.elapsedSeconds ?? 0;
  await page.evaluate((milliseconds) => window.advanceTime?.(milliseconds), Math.max(0, 2200 - elapsedAtDetection * 1000));
  ...
  if (!sameCamera(beforeDestruction?.camera, destruction?.camera)) throw new Error('The battle camera moved while following the falling mothership.');
  ...
  if (!sameCamera(beforeDestruction?.camera, impact?.camera)) throw new Error('The battle camera moved before the destruction sequence completed.');
  await page.locator('.battle-screen').waitFor({ state: 'visible' });
  ...
  await writeFile(`${outputDirectory}/result.json`, JSON.stringify({ ok: true, outcome: campaign.pendingDebrief.outcome, forceFlameFallback, useNaturalFailure, destruction, impact, repair, resources: campaign.resources, errors }, null, 2));
} catch (error) {
...
function sameCamera(before, after) {
  ...
  return ['x', 'y', 'z', 'targetX', 'targetY', 'targetZ'].every((key) => Math.abs(before[key] - after[key]) <= 0.001);
}
