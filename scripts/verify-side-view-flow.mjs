...
try {
  await page.goto(`${baseUrl}/?battle-fast=1&battle-fallback=1`, { waitUntil: 'domcontentloaded' });
  ...
  await writeFile(`${outputDirectory}/result.json`, JSON.stringify({ ok: true, targetId: target.id, stagedOutcome: stagedCampaign?.pendingDebrief?.outcome ?? null, errors }, null, 2));
} catch (error) {
