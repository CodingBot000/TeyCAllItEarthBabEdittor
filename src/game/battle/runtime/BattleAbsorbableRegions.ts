...
const SPRITE_URL_BY_KIND: Record<AbsorbableKind, string> = {
  ORGANIC: '/assets/runtime/sprites/target-organic-shelter-with-crowd-y0-web.webp',
...
const ORGANIC_SHELTER_DAMAGED_WITH_CROWD_URL = '/assets/runtime/sprites/target-organic-shelter-damaged-with-crowd-y0-web.webp';
const ORGANIC_SHELTER_DAMAGED_EMPTY_URL = '/assets/runtime/sprites/target-organic-shelter-damaged-y0-web.webp';
...
export class BattleAbsorbableRegions {
  ...
  private readonly organicShelterDamagedWithCrowdTexture: Texture;
  private readonly organicShelterDamagedWithCrowdMaterial: StandardMaterial;
  private readonly organicShelterDamagedEmptyTexture: Texture;
  private readonly organicShelterDamagedEmptyMaterial: StandardMaterial;
  ...
  constructor(private readonly scene: Scene, private readonly language: 'ko' | 'en' = 'ko') {
    ...
    this.organicShelterDamagedWithCrowdTexture = this.texture(ORGANIC_SHELTER_DAMAGED_WITH_CROWD_URL);
    this.organicShelterDamagedWithCrowdMaterial = this.spriteMaterial('battle-region-shelter-damaged-with-crowd', this.organicShelterDamagedWithCrowdTexture);
    this.organicShelterDamagedEmptyTexture = this.texture(ORGANIC_SHELTER_DAMAGED_EMPTY_URL);
    this.organicShelterDamagedEmptyMaterial = this.spriteMaterial('battle-region-shelter-damaged-empty', this.organicShelterDamagedEmptyTexture);
  }
  ...
  sync(state: Readonly<CombatState>): void {
    ...
    for (const target of state.absorbableTargets) {
      ...
      visual.sprite.material = shelterOrganic
        ? target.shelterBreachState === 'DESTROYED'
          ? target.remainingAmount > 0 ? this.organicShelterDamagedWithCrowdMaterial : this.organicShelterDamagedEmptyMaterial
  ...
  dispose(): void {
    ...
    this.organicShelterDamagedWithCrowdMaterial.dispose();
    this.organicShelterDamagedWithCrowdTexture.dispose();
    this.organicShelterDamagedEmptyMaterial.dispose();
    this.organicShelterDamagedEmptyTexture.dispose();
  ...
  private createVisual(id: string, x: number, radius: number, kind: AbsorbableKind): RegionVisual {
    ...
    const visual = { root, pad, core, beacon, sprite, shadow, spriteTexture, spriteBaseScaleX, spriteBaseScaleY, labelPanel, baseScaleX: radius };
