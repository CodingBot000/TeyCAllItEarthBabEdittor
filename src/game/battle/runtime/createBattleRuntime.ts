...
export async function createBattleRuntime(canvas: HTMLCanvasElement, map: BattleMapDefinition, options: BattleRuntimeOptions = {}): Promise<BattleRuntime> {
  ...
  if (!editorSceneLoaded) createFallbackMothershipAndUnits(scene, map, mothershipGameplayRoot, fighterPoolRoot, dronePoolRoot, groundBattleRoot, options.debugControls === true);
  if (options.debugControls !== true) hideDebugPrototypes(fighterPoolRoot, dronePoolRoot, groundBattleRoot);
  const fighterVisuals = options.debugControls === true ? createFighterVisuals(scene, fighterPoolRoot) : [];
  ...
  const simulateStep = (deltaSeconds: number) => {
    ...
    if (fighterVisuals.length > 0) animatePrototypes(fighterVisuals, dronePoolRoot, groundBattleRoot, elapsed, deltaSeconds);
...
function createFallbackMothershipAndUnits(
  ...
  includeDebugPrototypes: boolean,
): void {
  ...
  if (includeDebugPrototypes) createAirPrototypes(dronePoolRoot, scene);
  const groundLaneDefinitions = getOrCreateNode(scene, 'GroundLaneDefinitions');
  groundLaneDefinitions.parent = groundBattleRoot;
  if (includeDebugPrototypes) createGroundPrototypes(groundBattleRoot, scene);
}
