...
function CityDetails({ city, campaign, playable, canEngage, onMove, onEngage }: { city: CityDefinition; campaign: CampaignState; playable: boolean; canEngage: boolean; onMove: () => void; onEngage: () => void }) {
  ...
  return <div className="city-details" data-control-state={conquest.controlState}>
    ...
    <div className="city-title-row"><div><span className="city-accent" style={{ background: city.colorAccent }} /> <span className="city-country">{country ? displayCountryName(country, language) : city.country} / {city.admin1Name || city.admin1Code}</span><h3>{displayCityName(city, language)}</h3></div><span className="city-code">{city.countryId}-{city.id.slice(0, 3).toUpperCase()}</span></div>
    ...
    <div className="panel-actions">{!playable ? <button className="primary-button wide" disabled>{t('map.notAvailable')} <span>⊘</span></button> : canEngage ? <button className="primary-button wide" onClick={onEngage}>{t('map.enterAirspace')} <span>↗</span></button> : <button className="primary-button wide" onClick={onMove} disabled={Boolean(campaign.currentCityId === city.id || campaign.activeTransit)}>{t('map.moveToNode')} <span>→</span></button>}</div>
